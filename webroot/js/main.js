...
function getOCR(id,url) {
	var ocrurl = '/projects/ocr/';
	...
	$.ajax({
		...
		success: function( response ) {
			$('#loading-image').hide();
			if (response.profane > .5) {
				for (index = 0; index < response.profane_words.length; index++) { 
					wordlist = wordlist+' '+response.profane_words[index]; 
				}		
				$("#ocrResult_"+id).text(" - Profanity Found: "+wordlist);
				$("#ocrResult_"+id).css('color', 'red');
			} else {
				$("#ocrResult_"+id).text(" - Profanity Not Found");
				$("#ocrResult_"+id).css('color', 'green');
			}
		},
       	error: function(response) {
	       	$('#loading-image').hide();
			$("#ocrResult_"+id).text(" - Error checking this image");
			$("#ocrResult_"+id).css('color', 'red');
		}
 	});
}
