...
<div id="mod" class="<?=isset($tagged) ? $tagged : '';?>">
	...
	<div class="slide s<?=isset($numImages) ? $numImages : count($images);?><?= $project->scroll_vert ? ' vertical' : '';?>">
...
<iframe width="500" height="688" src="<?=$image['iurl']?>embed" frameborder="0"></iframe>				
			...
			?>
				...
				<a id="link_<?=$imagekey?>" href="javascript:void(0)" class="<?=strtolower($imgdef_page['name'])?>" data-imgid="<?=$imagekey?>" style="background-color:<?=$escalated  == 1?$esccolor:$imgdef_page['color']?>">
...
<iframe width="500" height="688" src="<?=$image2['iurl']?>embed" frameborder="0"></iframe>	
			...
			?>
				...
				<a id="link_<?=$imagekey2?>" href="javascript:void(0)" class="<?=strtolower($imgdef_page['name'])?>" data-imgid="<?=$imagekey2?>" style="background-color:<?=$escalated == 1?$esccolor:$imgdef_page['color']?>;">
					<h4 id="status_<?=$imagekey2?>"><?=$escalated==1?$escname:$imgdef_page['name']?></h4>
					<?= $project->scroll_vert ? '<div class="scroll">' : '';?>			
			...
			<input type="hidden" id="val_<?=$imagekey2?>" name="<?=$imagekey2?>" value="<?=$escalated  == 1?$escvalue:$imgdef_page['statval']?>" class="status" /> 
	...
	<?php } else { ?>
		<div id="last" class="slide">
		    <h3 id="lastSlide">No more images to moderate. <br>Looking for more....</h3>
		</div>
   <?php } ?>
   <?php } ?>
</div>
